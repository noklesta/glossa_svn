...
if ($c =~ m/^OMC3_/) {
 ...
 `/usr/local/bin/cwb-encode -xsB -f $tab -d $dat_dir -s -D -P lemma -P pos -P case -P type -P poss_dia_defin -P tense_grade_gender -P person_inf -P number_mood -P derivation -P clitic -P polarity_attributive -P syntax -S s:O+id -S text:O+id`;
}
